...
{
    var stringPassword, arrayBadPasswords, stringOutput, X
 ...
 /*The following lines create the password
  validation portion. It verifies that the
   password is 8 characters in length and 
   also that it is not in the bad pasword list */

    if (stringPassword.length < 8)
    {
        stringOutput = "Invalid length. Password must be at least 8 characters"
    }
    else
    {
        for (X=0; X < arrayBadPasswords.length; X++)
        {
            ...
            {
                stringOutput ="This is a poor password choice that may be easily guessed. Please choose a different password."
            }
        }
    }

/* The following section verifies that there
are no errors, and displays an acceptance
message that the password is 8 or more characters
and not in the blacklist
*/
if(stringOutput === "")
{
stringOutput = "This password is acceptable"
}
